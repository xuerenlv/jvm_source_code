...
/**
 * <p>
 * An <code>SslRMIServerSocketFactory</code> instance is used by the RMI
 * runtime in order to obtain server sockets for RMI calls via SSL.
 * </p>
 *
 * <p>
 * This class implements <code>RMIServerSocketFactory</code> over
...
public class SslRMIServerSocketFactory implements RMIServerSocketFactory {
	...
	/**
	 * <p>
	 * Creates a new <code>SslRMIServerSocketFactory</code> with
	 ...
	 *            enable on SSL connections accepted by server sockets created
	 *            by
	 ...
	 *            enable on SSL connections accepted by server sockets created
	 *            by
	 *            this factory, or <code>null</code> to use the protocol
	 *            versions
	 *            that are enabled by default
	 *
	 ...
	 *                suites named by the <code>enabledCipherSuites</code>
	 *                parameter is
	 *                not supported, when one or more of the protocols named by
	 *                the
	 *                <code>enabledProtocols</code> parameter is not supported
	 *                or when
	 *                a problem is encountered while trying to check if the
	 *                supplied
	...
	/**
	 * <p>
	 * Creates a new <code>SslRMIServerSocketFactory</code> with the
	 * specified <code>SSLContext</code> and SSL socket configuration.
	 * </p>
	 *
	 * @param context the SSL context to be used for creating SSL sockets.
	 *            If <code>context</code> is null the default
	 *            <code>SSLSocketFactory</code>
	 *            or the default <code>SSLServerSocketFactory</code> will be
	 *            used to
	 ...
	 * @since 1.7
	 */
	public SslRMIServerSocketFactory(SSLContext context, String[] enabledCipherSuites, String[] enabledProtocols,
		...
		if (this.enabledCipherSuites != null || this.enabledProtocols != null) {
			try {
				...
				final String msg = "Unable to check if the cipher suites " + "and protocols to enable are supported";
	...
	/**
	 * <p>
	 * Returns <code>true</code> if client authentication is
	 ...
	 *
	 * <p>
	 * Two <code>SslRMIServerSocketFactory</code> objects are equal
